...
- [ ] **1.1 Preparar ambiente de desenvolvimento**
  - [ ] 1.1.1 Criar o diretório `finanpy` e inicializar o repositório Git (`git init`).
  - [ ] 1.1.2 Criar o ambiente virtual: `python -m venv .venv` e ativá-lo.
  - [ ] 1.1.3 Instalar Django: `pip install django`.
  - [ ] 1.1.4 Instalar flake8 para checagem de PEP 8: `pip install flake8`.
  - [ ] 1.1.5 Gerar `requirements.txt` com `pip freeze > requirements.txt`.
  - [ ] 1.1.6 Criar `.gitignore` contendo: `.venv/`, `__pycache__/`, `*.pyc`, `db.sqlite`, `static/css/output.css`, `bin/`, `.env`, `staticfiles/`.
  - [ ] 1.1.7 Criar `.flake8` com `max-line-length = 79` e `exclude = .venv,migrations`.

- [ ] **1.2 Criar o projeto Django**
  - [ ] 1.2.1 Executar `django-admin startproject core .` (projeto `core` na raiz, com `manage.py`).
  - [ ] 1.2.2 Conferir que `core/` contém `__init__.py`, `asgi.py`, `settings.py`, `urls.py` e `wsgi.py`.
  - [ ] 1.2.3 Converter aspas duplas geradas pelo Django para aspas simples em `core/*.py` e `manage.py`.

- [ ] **1.3 Criar as apps de domínio**
  - [ ] 1.3.1 `python manage.py startapp users` — usuários.
  - [ ] 1.3.2 `python manage.py startapp profiles` — perfis.
  - [ ] 1.3.3 `python manage.py startapp accounts` — contas bancárias.
  - [ ] 1.3.4 `python manage.py startapp categories` — categorias.
  - [ ] 1.3.5 `python manage.py startapp transactions` — transações.
...
- [ ] **1.4 Configurar `core/settings.py`**
  - [ ] 1.4.1 Adicionar as apps em `INSTALLED_APPS` na ordem: apps do Django, depois `users`, `profiles`, `accounts`, `categories`, `transactions`.
  - [ ] 1.4.2 Configurar `DATABASES` com SQLite apontando para `BASE_DIR / 'db.sqlite'`.
